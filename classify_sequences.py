"""
🧬 CLASIFICACIÓN DE SECUENCIAS DE ADN CON CONFIANZA
Versión optimizada para Kaggle Notebooks

Modos de uso:
1. Con embeddings pre-calculados (RÁPIDO)
2. Sin embeddings (calcula on-the-fly)
3. Con filtro de confianza mínima
"""
...
print("="*80)
print("🧬 CLASIFICACIÓN DE SECUENCIAS CON CONFIANZA")
print("="*80)
...
MIN_CONFIDENCE = args.confidence_method 
#"""
...
K_NEIGHBORS = 10       # Número de vecinos para calcular confianza
K_PREDICT = 1          # Vecinos para predicción (típicamente 1)
...
if not USE_PRECOMPUTED_EMBEDDINGS and not COMPUTE_EMBEDDINGS:
    print("❌ Error: Debe activar USE_PRECOMPUTED_EMBEDDINGS o COMPUTE_EMBEDDINGS")
    raise ValueError("Configuración inválida")

if COMPUTE_EMBEDDINGS and not MODEL_NAME:
    print("❌ Error: MODEL_NAME requerido cuando COMPUTE_EMBEDDINGS=True")
    raise ValueError("MODEL_NAME no especificado")

if USE_PRECOMPUTED_EMBEDDINGS and not INPUT_EMBEDDINGS:
    print("❌ Error: INPUT_EMBEDDINGS requerido cuando USE_PRECOMPUTED_EMBEDDINGS=True")
    raise ValueError("INPUT_EMBEDDINGS no especificado")

# Mostrar configuración
print(f"\n📋 Configuración:")
print(f"   Modo: {'Pre-computed embeddings' if USE_PRECOMPUTED_EMBEDDINGS else 'Compute on-the-fly'}")
...
if 'MIN_CONFIDENCE' in locals() and MIN_CONFIDENCE > 0:
    print(f"   Min confidence: {MIN_CONFIDENCE}")

# ============================================
# INICIALIZAR DEVICE Y MODEL
# ============================================
device = None
model = None
tokenizer = None
...
def calculate_entropy_confidence(labels):
    """
    Confianza basada en entropía de Shannon.
    Menor entropía = mayor confianza
    Retorna: score 0-1 (1 = acuerdo perfecto)
    """
...
def calculate_agreement_confidence(labels, prediction):
    """
    Confianza como fracción de vecinos que concuerdan.
    Retorna: score 0-1
    """
    if not labels:
        return 0.0
...
def calculate_distance_confidence(distance):
    """
    Confianza basada en distancia al vecino más cercano.
    Menor distancia = mayor confianza
    Para COSINE: distancia típicamente [0, 2]
    Retorna: score 0-1
    """
...
def calculate_combined_confidence(entropy_conf, agreement_conf, distance_conf):
    """
    Combina múltiples métricas de confianza.
    Promedio ponderado.
    """
    w_entropy = 0.4
    w_agreement = 0.4
...
def classify_sequence(embedding, client):
    """
    Clasifica una secuencia y calcula scores de confianza.
    """
    try:
    ...
    # Cálculo de confianza por nivel taxonómico
    confidences = {}
    entropies = {}
    agreements = {}
    ...
    for level in HIERARCHY:
        ...
        # Seleccionar método de confianza
...
for i in tqdm(range(len(df)), desc="   Classifying"):
    ...
    if result:
        ...
        # Agregar predicciones y confianza por nivel
        for level in HIERARCHY:
        ...
        # Filtrar por confianza mínima si está configurado
        if 'MIN_CONFIDENCE' in locals() and MIN_CONFIDENCE > 0:
